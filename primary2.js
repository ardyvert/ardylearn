//code to execute simple addition with informative tips using the while loop
function add(){
window.location="index.html";
alert("Welcome to LEARN ADDITION.Lesson program coded by Burton Chulu,Cell +263 785 011 524,Email burtchulututorials@gmail.com");
n1=2;
n2=1;
while(n2<11){a=n1+n2;alert(n1+" + "+n2+" = "+
a);n2++;}
alert("Have a nice day and we hope we helped you learn something today");
}
//code to change body background color on onmouseover event
document.addEventListener("mouseover",function (){
document.body.style.backgroundColor="green";});
//code to execute subtraction using the while loop
function subtract(){
window.location="index.html";
alert("Welcome to LEARN SUBTRACTION.Lesson program coded by Burton Chulu,Cell +263 785 011 524,Email burtchulututorials@gmail.com");
n1=20;
n2=1;
while(n1>11){a=n1-n2;alert(n1+" - "+n2+" = "+a);n2++;n1--;}
alert("Have a nice day and we hope we helped you learn something today");
}
//code to change button element background color on mouseover event using the addEventListener method
document.addEventListener("mouseover",function (){
document.getElementById("d5").style.backgroundColor="red";});